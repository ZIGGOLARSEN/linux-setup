...
export PATH=~/.local/bin:~/bin:/snap/bin:/usr/sandbox/:/usr/local/bin:/usr/bin:/bin:/usr/local/games:/usr/games:/usr/share/games:/usr/local/sbin:/usr/sbin:/sbin:$PATH
...
case "$TERM" in
xterm*|tmux*|rxvt*)
  file=$(ps -ef | grep openvpn | grep -v grep | head -1 | rev | awk '{print $1}' | rev)
  if [ ! -z $file ]; then
      VPN=$(cat $file | grep remote | awk -F\. '{print $1}' | head -1 | cut -d ' ' -f 2)
  else
      VPN=''
  fi
    IP=$(ip -4 -o addr show wlo1 | awk '{print $4}' | sed 's/\/.*$//g')
    if [ ! -z "$VPN" ]; then
